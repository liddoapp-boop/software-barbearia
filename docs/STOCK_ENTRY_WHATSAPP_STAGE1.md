...
O processo executa um warm-up real com FFmpeg, `whisper-cli`, modelo e VAD antes de disponibilizar áudio. `serviceAvailable` só fica verdadeiro após esse warm-up; em caso de falha, o HTTP continua disponível para texto, mas áudio falha fechado sem executar o orquestrador ou alterar estado. Cada mídia é limitada por tamanho e duração. O provider grava a entrada em diretório temporário, conclui e valida a conversão PCM 16 kHz antes de iniciar o Whisper, e remove os artefatos em `finally`. O orçamento local padrão é 45 s e pode ser ajustado por `LOCAL_WHISPER_TIMEOUT_MS` entre 20 s e 120 s. Logs estruturados separam entrada, FFmpeg, Whisper, transcrição e limpeza, com duração, exit code, motivo seguro e fingerprints; nunca registram bytes, base64, transcrição integral, telefone completo, segredo ou URL de mídia.

Para diagnóstico ponta a ponta com um arquivo já capturado, sem efeito operacional, execute `npm run audio:doctor -- --input <arquivo.ogg>`. O comando usa a mesma configuração e o mesmo provider da aplicação, grava apenas relatório e transcrição em `.runtime/audio-diagnostics/` (ignorada pelo Git) e informa fingerprints sanitizados. Acrescente `--webhook` para passar o arquivo pelo webhook em memória, bloquear toda rede real, repetir o mesmo `messageId` e comprovar deduplicação e ausência de alterações em estoque, vendas, movimentos e financeiro. Falha de entrada vazia, FFmpeg, Whisper, timeout, ausência de fala e transcrição vazia aparecem como razões distintas. Preserve o relatório antes de tentar novamente; os temporários internos do provider são removidos após cada execução.
