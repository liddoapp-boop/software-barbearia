-- Macro 235.2.1 - reconciliacao pre-baseline
-- Escopo: aplicar somente em clone restaurado do banco local principal.
-- Proibido aplicar diretamente em localhost:5432/barbearia nesta fase.
...
BEGIN
  IF current_database() = 'barbearia' THEN
    RAISE EXCEPTION 'recusado: este script nao pode rodar no banco principal barbearia';
  END IF;

  IF current_database() NOT LIKE 'barbearia_reconcile_235_2_1_%' THEN
    RAISE EXCEPTION 'recusado: banco % nao e clone de reconciliacao 235.2.1', current_database();
  END IF;
